...
class Pathfinding(object):
	...
	def get_Width(self):
		return self._cells.GetLength(0)

	Width = property(fget=get_Width)

	def get_Height(self):
		return self._cells.GetLength(1)
	...
	def Draw(self, driver, cellTexture):
		...
		for i in xrange(0,self._cells.GetLength(0),1):
			for j in xrange(0,self._cells.GetLength(1),1):
				k = -1 - self._cells[i,j]
				if k >= 0 and k < srcRect.Length:
					self._batchSrcRect[i * self._cells.GetLength(1) + j] = srcRect[k]
		driver.Draw2DImageBatch(cellTexture, self._batchDestPos, self._batchSrcRect)

	def SetCell(self, x, y, c):
		if x < 0 or x >= self._cells.GetLength(0) or y < 0 or y >= self._cells.GetLength(1):
			return 
	...
	def FindPath(self):
		...
		w = self._cells.GetLength(0)
		h = self._cells.GetLength(1)
		# clean up prev path
		for i in xrange(0,w,1):
			for j in xrange(0,h,1):
				if self._cells[i,j] == -5:
					self._cells[i,j] = -1
		# build path data
		c = self._cells.Clone()
